...
const ulanArtistsFile = './data/ULAN/ulanArtists.jsonl';
const ulanCorporateBodiesFile = './data/ULAN/ulanCorporateBodies.jsonl';

const outputFilename = './data/artistTerms.jsonl';
...
async function correlate() {
  ...
  for await (const line of rl) {
    ...
    if (obj?.primaryConstituent?.length > 0) {
      ...
      let foundTerm = checkMatchedArtistTerms(
        preferredTerms,
        tmsDateObj
      );
      if (!foundTerm) {
      ...
      if (foundTerm) {
        ...
        if (foundTerm.alternates?.length > 0) {
          ...
          for (const alt of foundTerm.alternates) {
            ...
            if (cleanArtist.includes(cleanAlt) || cleanPreferred.includes(cleanAlt)) continue;
